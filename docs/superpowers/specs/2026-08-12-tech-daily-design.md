...
信息以欧美前沿动态为主，内容保持英文原文，页面框架使用中文；GitHub 仓库描述、AI 新闻标题、工具标题附一行机器翻译的中文参考（带“译：”前缀），原文保持权威。
...
- GitHub 仓库卡片在英文描述下方、新闻/工具条目在英文标题下方附一行机器翻译中文（Google 公共翻译接口，MyMemory 兜底；失败则省略该行）。
